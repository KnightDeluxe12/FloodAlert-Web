...
void loop() {
  ...
  avg_val /= avg_size;
  float min_value = 8.8e6;   // minimum value of the raw data higher means low value vice versa
  float max_value = 11.7e6;  // maximum value of the raw data
  float calibrationFactor = 12.3;
  ...
  if (pressureInCM >= -10 && pressureInCM <= 4) {            //board 2: 4 and 4.5
    pressureInCM = 0;
  } else if (pressureInCM >= 4.5) {
  ...
  float filteredVoltage = ((measuredVoltage - (3.3)) / (4.2 - (3.3))) * 100;
