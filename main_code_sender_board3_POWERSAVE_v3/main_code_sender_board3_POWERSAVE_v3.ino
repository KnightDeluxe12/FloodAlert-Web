...
// const int SLEEP_TIME = 2 * 60;       // Sleep time in milliseconds (5 minutes)
...
void OnDataRecv(const uint8_t *mac_addr, const uint8_t *incomingData, int len) {
  ...
  switch (type) {
    case DATA:  // we received data from server
      ...
      // Serial.print("Setpoint water level = 0");
...
void task1(void *parameter) {
  for (;;) {
    ...
    // while (millis() - startTime < RUN_TIME) {
      ...
      avg_val /= avg_size;
      float min_value = 8.5e6;   // minimum value of the raw data higher means low value vice versa
      float max_value = 11.8e6;  // maximum value of the raw data
      float calibrationFactor = 10.8;

      float pressure = ((avg_val - min_value) / (max_value - min_value) * 1000) * calibrationFactor;
      pressureInCM = pressure / (1000.0 * 1.0 * 9.80665);  // result is in meters
      pressureInCM = pressureInCM * 100;                   // convert to centimeters
      if (pressureInCM >= -10 && pressureInCM <= 2) {      //board 2: 4 and 4.5
        pressureInCM = 0;
      } else if (pressureInCM >= 2.5) {
